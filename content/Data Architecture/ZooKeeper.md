...
zookeeper是apache开源提供的一个分布式的服务协调框架，主要用来解决分布式集群中应用系统的_**一致性**_问题，例如怎样避免同时操作同一数据造成**脏读的问题。**ZooKeeper 本质上是_**一个分布式的小文件存储系统**_。主要的作用，是保证让其他的框架正常运行，本身独立运行没有太大的意义，主要是对外提供一些辅助服务，帮助其他的框架正常运行。本质上是一个分布式的小文件存储系统：不擅长存储海量的数据
