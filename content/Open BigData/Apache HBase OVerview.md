...
> Hbase（[[Apache Hadoop Overview|Hadoop]] on database）是基于[[HDFS]]进行数据的存储，具有高可靠. 高性能. 列存储. 可伸缩. 实时读写的nosql数据库。它可以存储海量数据，并且后期查询性能很多，可以实现上亿条数据的秒级返回。
...
graph TD
  ...
  E["Column Family 1"]
  F["Column Family 2"]
  G["Cells(Key-Value Pairs)"]
  H["Cells(Key-Value Pairs)"]
  I["(row,column,timestamp) → value"]
...
![[content/Open BigData/Apache Hadoop/images/apache-hbase-logo.png]]
