...
draft: true
publish: true
description: HDFS 分布式文件系统架构全景：NameNode 元数据管理、BlockManager 副本调度、DataNode 存储体系、数据读写管道、Federation 联邦扩展与高可用机制
language: CN
---
...
| **ZKFailoverController**（ZKFC） | 通过 [[Data Architecture/ZooKeeper\|ZooKeeper]] 监控 NameNode 健康状态，触发自动故障转移 |
