...
draft: false
---
MapReduce是一种**分布式计算框架**，可以用于大规模数据处理。它是由Google提出的一种计算模型，可以通过横向扩展来处理PB级别的数据。Hadoop是实现MapReduce的一种开源框架，它通过[[HDFS]]存储数据，并通过MapReduce处理数据。
...
- [HowToDebugMapReducePrograms](https://cwiki.apache.org/confluence/display/HADOOP2/HowToDebugMapReducePrograms)
- Examples
