...
数据被切成多个分片，不同节点可以并行处理。例如一张 100 亿行订单表：
...
最后再把结果汇总。这就是 MPP、Spark、Flink、Hive 等系统能够处理大数据的基础。
...
如果查询需要跨节点关联，就会发生网络传输，也就是 **Shuffle**。例如：
...
如果某些 key 的数据特别多，会导致部分节点压力巨大。例如某个大客户贡献了 40% 订单量：
...
**Shared-data**，中文可以理解为：**共享数据架构**。它的核心思想是：

多个计算节点共享同一份底层数据，计算和存储解耦。也就是说，数据

不是绑定在某个计算节点本地，而是放在一个共享存储层中。
...
计算节点可以按需启动和释放。例如白天查询多，开 20 个计算节点；晚上查询少，缩到 2 个节点。这对云原生数据平台非常重要。
...
优点是每个人可以并行做饭，速度快。缺点是如果 A 需要 B 的食材，就要互相搬来搬去，成本高。
...
优点是食材统一管理，厨师可以随时增减。缺点是如果所有厨师同时去仓库拿东西，仓库可能堵住。
...
**Shared-nothing** 强调：数据和计算分布在各个节点上，通过分片实现高性能并行计算。

**Shared-data** 强调：多个计算节点共享同一份数据，计算和存储解耦，适合云原生和湖仓一体。
