...
function byDateFallback(cfg: GlobalConfiguration, f1: QuartzPluginData, f2: QuartzPluginData): number {
...
export function byDateAndAlphabetical(cfg: GlobalConfiguration): SortFn {
  return (f1, f2) => {
    const p1 = (f1.frontmatter?.priority as number) ?? 0
    const p2 = (f2.frontmatter?.priority as number) ?? 0
    if (p1 !== p2) return p2 - p1

    return byDateFallback(cfg, f1, f2)
  }
}
